...
export function AudioLibrary({ onPlay, onAddToQueue }: AudioLibraryProps) {
      ...
      try {
        ...
        if (albumsData && albumsData.length > 0) {
          ...
          const fullAlbumsStructure = albumsData.map((album: any) => {
            ...
            // إسناد إحصائيات استماع وتحميل افتراضية واقعية لكل قصيدة إذا لم تكن مسجلة بقاعدة البيانات
            const tracksWithStats = albumTracks.map((track: any) => {
              const baseListens = track.listens_count ?? 0;
              const baseDownloads = track.downloads_count ?? 0;
              return {
                ...track,
  ...
  const incrementTrackStat = async (trackId: string | number, type: "listens" | "downloads") => {
    ...
    if (error) {
      console.error("حدث خطأ في تحديث الإحصائيات:", error);
      return;
    }
              ...
              {loading ? (
                ...
                <>
                  <TabsContent value="sorrow" className="mt-0 focus-visible:outline-none">
                    <AlbumGrid albums={visibleAlbums} onPlay={onPlay} onAction={handleAction} onIncrementStat={incrementTrackStat} />
                  </TabsContent>
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1 shrink-0">
                                <Button
                                  ...
                                  size="icon"
                                  onClick={() => {
                                    onIncrementStat(track.id, "downloads");
